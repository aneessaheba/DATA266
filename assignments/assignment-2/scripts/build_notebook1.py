...
# ============================================================ Title
md("""# DATA 266 — Embedding Transfer Learning & RAG Pipeline

**Part 1:** Embedding-based transfer learning on IMDB reviews (Word2Vec fine-tuning)
...
model, fine-tuning it on real IMDB review text, and building a working RAG pipeline with a
local embedding model + local LLM (no paid API keys required).
""")
...
model (pretrained on ~100B words of Google News). It was downloaded once via
...
md("""## 1.2 Top-3 nearest neighbors (before fine-tuning)
...
50,000 labeled + unsupervised reviews). For a tractable fine-tuning runtime, we randomly
subsample 15,000 reviews from the combined train + unsupervised splits (75,000 reviews total
...
# ============================================================ 1.4 Fine-tune
md("""## 1.4 Fine-tune the pretrained embeddings on IMDB text

We build a new `gensim.models.Word2Vec` model over the IMDB vocabulary (skip-gram, 300-dim to
match the pretrained space), then **initialize every overlapping word's vector with its
pretrained Google-News vector** before continuing training (5 epochs) on the IMDB corpus. This
is the standard "continue training from pretrained vectors" transfer-learning recipe.

*(Note: gensim's built-in `KeyedVectors.intersect_word2vec_format()` helper is broken under
NumPy ≥ 2.0 — `np.fromstring` in binary mode was removed. We get the identical effect by
copying the pretrained vectors directly into the new model's vector matrix for every word that
exists in both vocabularies, before training.)*
""")
...
md("""## 1.5 Top-3 nearest neighbors (after fine-tuning)
""")
...
md("""**Observations.** After fine-tuning on movie reviews, every target word's neighbors shift
toward its **domain-specific, film-critique** sense:

- `cast` → `supporting`, `ensemble` (an ensemble/supporting **cast** of actors)
- `score` → `ennio`, `morricone` (the famous film composer *Ennio Morricone* — a musical **score**)
- `plot` → `story`, `storyline` (narrative **plot**, not a literal plot of land)
- `screen` → stays close to `screens`/`onscreen` (already film-adjacent even pre-fine-tuning)
- `review` → `reviews`, `comment`, `comments` (review-discussion context)

This is exactly the expected effect of transfer learning: the general-purpose semantics get
...
**Figure 1** projects all 5 words' *own* vectors (before and after fine-tuning) into 2D with
t-SNE, connecting each word's before/after position with an arrow.

**Figure 2** zooms in on `plot` specifically, plotting it together with its top-3 neighbors in
*both* spaces — this is the clearest single-word illustration of the domain shift.
""")
...
ax.set_title("Figure 1: t-SNE shift of each word's own vector (before -> after fine-tuning)")
ax.legend()
fig.tight_layout()
plt.show()
""")
...
ax2.set_title(f"Figure 2: '{target}' and its top-3 neighbors, before vs after fine-tuning")
fig2.tight_layout()
plt.show()
""")

md("""`plot` visibly moves **away** from its morphological/literal neighbors (`plots`, `plotting`,
`Plot`) and **toward** its narrative-sense neighbors (`story`, `storyline`, `plotline`) — a
clean, visual confirmation of the domain-adaptation effect from Section 1.6.
""")

# ============================================================ 1.8 Drift table
md("""## 1.8 Per-word vector drift: cosine similarity of original vs. fine-tuned vector

For each of the 5 words, we compute the cosine similarity between its *own* vector before and
after fine-tuning (not the neighbor lists — the raw vector itself). A **lower** similarity means
the word's meaning shifted **more** during fine-tuning.
""")
...
md("""**`review` shifted the most** — makes sense: in general news text "review" spans many
senses (performance review, code review, review of evidence), so IMDB's narrow, repetitive
"movie review / reader comment" usage pulls it hardest away from its original position.

**`cast` shifted the least** — "cast" already has a strong film/theater sense in general English
(a movie's *cast*), so IMDB usage reinforces rather than redefines its pretrained meaning.
""")
...
- **Pipeline:** built from **explicit** components — `PromptTemplate` + retriever + LLM call —
  rather than a single pre-built chain like `RetrievalQA.from_chain_type`.
""")
...
We deliberately avoid a single pre-built chain (e.g. `RetrievalQA`). Instead we wire the three
components together ourselves so every step is visible: retrieve → format context → fill prompt
template → call the LLM.
""")
...
def run_rag(question, retriever_):
    # do_sample=False -> greedy decoding, so the LLM's answer is deterministic and
    # reproducible across re-runs (the default sampling config would otherwise
    # give a different answer on every execution for the same context/question).
...
md("""## 2.5 Five questions, run through retriever + LLM

Each question below is answerable directly from the 10 Wikipedia pages. For each, we display the
top-3 retrieved chunks and the LLM's generated answer.
""")
...
# ============================================================ 2.6 reconfig
md("""## 2.6 Re-run 2 of the 5 questions with a different chunk configuration

We rebuild the vector store with **chunk_size=1000, chunk_overlap=100** (larger, more
overlapping chunks) and re-run the questions about *Jurassic Park* and *Titanic* — the two
questions most likely to be sensitive to chunk boundaries (the Jurassic Park plot summary and
the Titanic intro paragraph both contain multiple named entities close together).
""")
...
| Question | Original config (500/50) | Reconfigured (1000/100) |
|---|---|---|
| Jurassic Park paddock dinosaur | Top-3 chunks miss the T. rex-escape sentence entirely (retrieved the title, a dinosaur *list* header, and the *opening* Velociraptor-kills-a-handler scene) → LLM hallucinates **"Velociraptor"** | Chunk 1 now *does* contain "...allows a Tyrannosaurus rex to escape and attack the touring group" (bigger chunks merged it with surrounding plot text) → retrieval fixed, **but** the LLM still answers **"Dilophosaurus"** (a different dinosaur mentioned two sentences later in the same chunk) |
| Titanic ship & year | Chunk 1 contains "...sinking of RMS Titanic in 1912" → correct answer | Same chunk 1 still contains the fact → same correct answer, unaffected by the reconfiguration |

**Take-away:** larger chunks improved retrieval for the Jurassic Park question (the relevant
fact is now present in the top-3), but it did **not** fix the final answer — the generation step
...
For each of the 5 original questions we manually located the exact source passage containing the
correct answer, then checked whether any of the top-3 retrieved chunks contains it, and if so at
what rank.
""")

code("""manual_assessment = pd.DataFrame([
    {"question": "Inception director & composer",
    ...
    {"question": "Andy Dufresne's profession",
     ...
     "found_in_top3": "Yes", "rank_of_first_relevant_chunk": 1},
    {"question": "Titanic ship & sinking year",
...
With chunk_size=500/overlap=50, the sentence describing the T. rex's paddock escape
(*"Most of the park's electric fences have been deactivated, which allows a Tyrannosaurus rex to
escape and attack the touring group."*) falls into a **chunk that never surfaces in the top-3**
for this query — the retriever instead returns the page title, a bare `List` section header, and
an *earlier* scene where a Velociraptor kills a park worker. Because the word "Velociraptor" *is*
present in the retrieved context (from that unrelated scene) while the actual answer is not, the
small LLM confidently answers **"Velociraptor"** — a textbook case of *the correct document not
...
### Failure 2 — Correct context retrieved, but the LLM still answers incorrectly (Jurassic Park, reconfigured)

After widening the chunk size to 1000/100, the correct sentence about the Tyrannosaurus rex *is*
now inside the top-1 retrieved chunk. Despite the right fact being present, the LLM answers
**"Dilophosaurus"** — a *different* dinosaur that is mentioned two sentences later in the very
same chunk (Nedry is killed by "a venom-spitting Dilophosaurus"). This is a pure **generation
failure**: the small instruction-tuned model (0.5B parameters) had the right context but failed
to correctly attribute the "escapes its paddock" action to the right entity when multiple
dinosaur names compete within the same passage. It illustrates that fixing retrieval does not
guarantee a correct final answer, and that reader/LLM capacity is a separate failure axis from
retrieval quality.

### Contributing factor — ambiguous / overlapping entity names

Both failures share a root cause: Jurassic Park's plot passage densely packs multiple named
dinosaur species into adjacent sentences (Velociraptor, Tyrannosaurus rex, Dilophosaurus,
Brachiosaurus), which is a specific case of "ambiguous entity names causing retrieval/generation
confusion" — here it's not movie-title ambiguity but *in-document* entity crowding that confuses
both the retriever's chunk selection and the reader's attribution.
""")
...
| Word with most embedding drift (Part 1) | **`review`** (cos_sim = lowest) |
| Word with least embedding drift (Part 1) | **`cast`** (cos_sim = highest) |
| RAG failures documented | **2** (retrieval miss + hallucination; correct context + generation error) |
""")
